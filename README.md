...
| | Vega Agent | OpenClaw | Hermes Agent | OpenHuman |
|---|---|---|---|---|
| **Focus** | Personal AI agent, local-first | Multi-channel personal AI assistant | General-purpose AI agent framework | Privacy-preserving AI agent |
| **Install** | `curl getvega.sh | bash` | `npm install -g openclaw` | Python package | Docker / cloud |
| **Language** | Python 3.11 | TypeScript / Node.js | Python | Python |
| **Privacy** | Scope enforcement + JSONL audit log | Sandbox-based (Docker/SSH) | Configurable | 4-layer provable boundary |
| **Memory** | Context Tree (SQLite + ChromaDB) with auto-pruning | Flat memory (files + embeddings) | Variable | Encrypted vector store |
| **Channels** | CLI + shell | 20+ channels (WhatsApp, Telegram, Discord, Signal, iMessage, etc.) | CLI + Discord + TUI | Dashboard only |
| **Knowledge** | Structured graph (root/branch/leaf) with typed edges | Skills + SOUL.md files | Optional | None |
| **Audit** | Built-in JSONL | Security policies (no audit log) | Configurable | External via Guild.ai |
| **Extensibility** | LangGraph workflows | Skills (ClawHub, 5,400+) | Plugin system | Protocol-based |
| **Models** | OpenRouter, OpenAI, local (pluggable) | All major providers (Anthropic, OpenAI, Ollama, ~20 more) | Multiple providers | Custom protocol |
| **Local-only** | Yes (`local_models_only`) | Yes (Docker sandbox) | Partial | Yes |
| **Community** | New | **373K stars, 77K forks, 50K+ commits** | 156K stars | 0 stars |
